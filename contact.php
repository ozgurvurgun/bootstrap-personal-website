...
if (isset($_POST["ad"], $_POST["mail"], $_POST["tel"], $_POST["konu"], $_POST["mesaj"], $_POST["security"])) {
    if ($secur == $_POST["security"]) {
        ...
        $konu = $_POST["konu"];
        $mesaj = $_POST["mesaj"];
        $ekle = "INSERT INTO iletisimtb(adsoyad, mail, telefon, konu, mesaj) VALUES ('" . $adsoyad . "','" . $email . "','" . $telefon . "','" . $konu . "','" . $mesaj . "')";
        if ($baglan->query($ekle) === TRUE) {
            echo "<script>alert('Mesajınız başarı ile gönderilmiştir. En kısa sürede dönüş yapacağım :)')</script>";
            unset($_SESSION["rand"]);
        } else {
        ...
        echo "<script>alert('Doğrulama kodunu hatalı girdiniz lütfen tekrar deneyin')</script>";
        unset($_SESSION["rand"]);
    }
    echo "<script>window.location.href='exitt.php'</script>";
}
